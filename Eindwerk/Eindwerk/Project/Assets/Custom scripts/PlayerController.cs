...
public class PlayerController : MonoBehaviour {
	...
	public int punch=0;
	public int maxpunch=20;
	public int kick=0;
	public int maxkick=20;
	public bool hurtcombo=false;
	public int hurttimer=0;
	public int maxhurt=20;
	private bool guardcombo=false;
	public int guardtimer=0;
	public int maxguard=10;
	private bool specialcombo=false;
	public int specialtimer=0;
	public int maxspecialtimer=75;
	...
	if(networkView.isMine)
	{
		if(Opponent!=null)
			{
				if(controller!=null && controller.isGrounded && characterstate!="punching"& characterstate!="punching2" && characterstate!="kicking"&& characterstate!="kicking2" && characterstate!="hurting" && characterstate!="guarding" && characterstate!="special")
				{
				networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"idle");
				}
			
				if (Input.GetButtonDown(jumpstring) && canControl && characterstate!="guarding" && characterstate!="special") // checks if the jumpbutton was pushed and sets the last time that it was pushed to prevent characters from jumping to quickly
				{
				networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"jumping");
				...
				{
					punch++;
					if(punch==maxpunch-1 || punch==(maxpunch*2)-1)
					{
						punch=0;
						punchcombo=false;
						if(characterstate!="special")
						{
				...
				{
					kick++;
					if(kick==maxkick-1 || kick==(maxkick*2)-1)
					{
						kick=0;
						kickcombo=false;
						if(characterstate!="special")
				...
				{
					guardtimer++;
					if(guardtimer==maxguard)
					{
						networkView.RPC("ChangeSpecial",RPCMode.AllBuffered,1);
						guardtimer=0;
					}
					if(health.currSpecial==0)
					{
						guardtimer=0;
						guardcombo=false;
						animation.CrossFade(Clips[0].name);
				...
				{
					specialtimer++;
					if(specialtimer==maxspecialtimer)
					{
					specialtimer=0;
					specialcombo=false;
					animation.CrossFade(Clips[0].name);
		...
		if(animation)
		{
			...
			else
			{
				...
				{
					animation[Clips[1].name].speed= 1.0f;
						...
						else
						{
								...
								else
								{
									...
									else
									{
										...
										{
										if(characterstate=="guarding")
										{
										animation[Clips[8].name].speed= 0.4f;
										animation.CrossFade(Clips[8].name);
										}
										else
										{
											if(characterstate=="special")
											{
												animation[Clips[9].name].speed= 0.4f;
												animation.Play(Clips[9].name);
											}
											else
											{
												animation[Clips[0].name].speed= 1.0f;
	...
	public void UpdateSmoothedMovementDirection()
	{
		...
		if(!canControl || characterstate=="guarding" || characterstate=="special") //checks if our guy can move (handy for intro's) 
		{
			h=0.0f; 
		}
		
		if(Input.GetButtonDown(punchstring) && canControl && characterstate!="special")
		{
			h=0.0f;
			if(punchcombo==false)
			{
			networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"punching");
			punchcombo=true;
			punch=0;
			}
			
			if(punch<maxpunch &&punch >=attackdelay)
				{
				networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"punching2");
				punch=maxpunch;
				}
				else
				{
					if(punch<=maxpunch*2 && punch >maxpunch+attackdelay)
					{
					networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"punching");
					punch=0;
					}
		...
		if(Input.GetButtonDown(kickstring) && canControl && characterstate!="special")
		{
		h=0.0f;
			if(kickcombo==false)
			{
			networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"kicking");
			kickcombo=true;
			kick=0;
			}
			if(kick<maxkick &&kick >=attackdelay)
				{
				networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"kicking2");
				kick=maxkick;
				}
				else
				{
					if(kick<=maxkick*2 && kick >(maxkick+attackdelay))
					{
					networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"kicking");
					kick=0;
					}
		...
		if(Input.GetButtonDown(specialstring) && health.currSpecial==health.maxSpecial)
		{
			h=0.0f;
			specialcombo=true;
			networkView.RPC ("ChangeCharacterState",RPCMode.AllBuffered,"special");
			networkView.RPC ("ChangeSpecial",RPCMode.AllBuffered, health.maxSpecial);
		}
		
		if(Input.GetButtonDown(guardstring) && canControl && characterstate!="special")
		{
			if(health.currSpecial!=0)
			{
			networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered, "guarding");
			guardcombo=true;
			}
		}
		if(Input.GetButtonUp(guardstring) && canControl && characterstate!="special")
		{
			networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered, "idle");
			guardcombo=false;
		}
		...
		if(controller.isGrounded ) // checks if our character is grounded
		{
			...
			if(h<0 && Opponent.position.x< transform.position.x && characterstate!="guarding" && characterstate!="special")
			{
				targetSpeed*= ForwardSpeed;// walking forwards when you are standing right
				if(characterstate!="jumping"&& characterstate!="punching"&& characterstate!="punching2" && characterstate!="kicking"&& characterstate!="kicking2"&&characterstate!="hurting" && characterstate!="guarding" && characterstate!="special")
					networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"walking");
			}
			else
			{
				if(h>0 && Opponent.position.x< transform.position.x && characterstate!="guarding"&& characterstate!="special")
				...
				else
				{
					...
					{
						if(Opponent.position.x == transform.position.x && characterstate!="guarding" && characterstate!="special")
						{ // when the positions are the same when jumping nothing should be changed.
						} 
						else
						{
						if(characterstate!="guarding" && characterstate!="special")
							{	
			...
			if(h==0.0f && characterstate!="jumping" && characterstate!="punching"&& characterstate!="punching2" && characterstate!="kicking"&& characterstate!="kicking2" &&characterstate!="hurting" && characterstate!="guarding" && characterstate!="special")
			{
				networkView.RPC("ChangeCharacterState",RPCMode.AllBuffered,"idle");
			}
	...
	[RPC]
	public void ChangeCharacterState( string state)
	{
		characterstate=state;
	}
	
	[RPC]
	public void ChangeSpecial(int s)
	{
		health.currSpecial-=s;
		if(health.currSpecial<0)
		{
		health.currSpecial=0;
		}
	}
}
